...
(b) 如果二十世纪左撇子的流行率有所增加，或者不同社会群体（死亡率不同）内的流行率发生了变化。这两种情况都很有可能。  
...
(c) 该分析会偏向于纳入那些很久以前出生的人，因为最近的球员大多仍然健在。左撇子在本世纪早期可能比现在更不常见。这两个事实意味着该分析会偏向于左撇子更早死亡。此外，排除仍在世者来分析平均死亡年龄是具有误导性的。正确的生存数据分析方法，也考虑了幸存者的数据，在第13章中有所描述。  
...
(d) 最好选取一批大约同时出生的人，例如同一年级的学生。为了获得合理的死亡比例，幸存者需要至少70岁。这种数据极不可能获得，而且一项前瞻性研究将耗时非常长！如果使用适当的方法进行分析，并且在分析中考虑出生年份，棒球数据将得出更有效的结果。  
...
7.1 图A7.1显示了数据的散点图。离总体趋势最远的点位于左下角，对应于患者19。该患者的肺动脉楔压实际为 $28 \mathrm{mmHg}$ 。  
...
正态分数可以在许多统计软件包中获得。否则，可以通过“反向”使用表B1获得。这些数据的正态图非常直，如图A7.2所示。  
...
8.3 另一种提问方式是，“一个组中的数字小于40或大于60的概率是多少？”。分配特定治疗的患者比例的抽样分布是二项分布。该分布的标准差是 $\sqrt{np(1 - p)}$。这里 $p = 0.5$，所以标准差是 $\sqrt{100 \times 0.5 \times 0.5} = 5$。使用正态近似（对于 $p = 0.5$ 非常好），所需的概率是对应于 $z = (60 - 50) / 5 = 2.0$ 的双尾面积，从表B2可知为0.0455，或大约 $5\%$。  
...
9.2 (a) 配对 $t$ 检验结果为 $t = 1.85$， $P = 0.08$。然而，该检验  
...
(c) 如果数据要通过方差分析进行分析，那么我们需要标准差相似（理论上，各组是来自具有相同标准差的总体的样本）。对标准误没有这样的要求，标准误部分取决于  
...
9.8 可以使用Mann-Whitney检验。由于它是一种基于秩次的检验，记录为 $>80.0$ 的截尾值都将具有相同的秩次。由于在这个值上有如此多的并列，因此进行并列调整将是可取的。此外，SI值显然具有偏斜分布，  
...
10.6 从表中可以清楚地看出，有些患者有不止一种习惯，这是可以预料的。因此，计算完整 $4 \times 3$ 表格的 $X^{2}$ 是不正确的。如果我们知道每位患者的习惯，  
...
(c) PPV是指呼吸频率 >50 的婴儿中患有LRI的比例，为 $26\%$。因此，如果所有呼吸频率高于该临界值的婴儿都接受抗生素治疗，其中 $74\%$ 的婴儿将是“不必要”的治疗。在此临界值下，敏感性为 $89\%$，这意味着 $89\%$ 的LRI婴儿将接受抗生素。因此 $11\%$ 的婴儿将不会获得抗生素。在较低的40次/分钟临界值下，接受抗生素治疗的LRI婴儿比例会更高 $(96\%)$，但只有 $7\%$ 接受治疗的婴儿患有LRI。应该清楚，临界值的选择必须基于非统计学考虑。  
...
(d) 如果所有高于临界值的婴儿都接受治疗，那么 $89\%$ 的LRI婴儿和 $8\%$ 的URI（上呼吸道感染）婴儿将接受治疗。假设LRI的比例为 $3\%$，则接受治疗的比例将是 $0.89 \times 0.03 + 0.08 \times 0.97 = 0.10$。因此，只有 $10\%$ 的婴儿将接受抗生素治疗，这将大大节省成本。即使使用40次/分钟的临界值，接受治疗的比例也将是 $0.96 \times 0.03 + 0.38 \times 0.97 = 0.40$，这将比现有政策的成本减少一半。  
...
14.5 (a) 对于每对牙医，通过汇总表格的相关行，每颗牙齿可以被归类为SS、SC、CS或CC。由此产生的 $2 \times 2$ 表格可以通过计算kappa值进行评估，如下所示：  
...
表中也显示了百分比一致性。因此，牙医1和牙医2之间的一致性最好。牙医3认为龋齿数量（1644颗）远多于其他两位观察者（339颗和496颗）。(b) kappa值为0.46通常不被认为是特别好的一致性，但牙医1和牙医2对所检查牙齿的 $97\%$ 达成了一致。这种差异是因为他们都认为绝大多数牙齿是健康的。在至少有一位牙医（1或2）认为龋齿的619颗牙齿中，他们仅对 $216$ 颗 $(35\%)$ 达成了一致。良好的一致性取决于具体情况，而不是kappa值（当然也不是P值—由于样本量巨大，上述所有kappa值都具有高度统计学意义）。  
...
15.1 治疗组基线值中可能影响患者预后的变量差异可能会影响试验结果。重要的是不平衡的程度—统计学意义是无关紧要的。在表3.5中，除了就诊时的疼痛外，差异微不足道。由于疼痛评分也是试验的结局指标，这种不平衡可能很重要。因此，分析疼痛从基线的变化而不是研究结束时的值可能是合理的。（事实上，在这项研究中，作者指出基线疼痛评分不具有预后价值。）当某个变量存在不平衡且其预后价值未知时，可以针对每个治疗组的该变量值来检查结局。预后变量的不平衡通过使用回归分析调整治疗组比较来处理，参见第15.4节。  
...
15.2 (a) 只有被分配到阿普洛尔的患者才会因β受体阻滞剂的禁忌症而退出。因此，这两组除了大小不相等外，将是不可比较的，试验结果将不可靠。  
...
15.3 (a) 这是一项非对照试验，这不是评估治疗的正确方法。这项研究的另一个非常糟糕的特点是研究是“开放的”，因此患者知道他们何时服用阿司匹林。此外，这项研究的样本量极小，尤其考虑到测量结果的高度变异性。  
...
(b) 标准差通过 $\mathrm{SE} \times \sqrt{13}$ 获得，对于三组观察结果分别为 36.1、26.7 和 19.8。因此，数据是偏斜的，尤其是在治疗前，因为标准差超过平均值的一半。  
...
(c) 该论文没有说明数据是如何分析的（如果有的话）。适当的分析将是配对 Wilcoxon 检验来比较任意两组数据，或者如果差异近似正态分布，则使用配对 $t$ 检验。双向方差分析可用于同时比较所有三组。然而，如前所述，这些分析的价值有限，因为没有对照组。  
...
显著性。置信区间会非常宽。仅仅根据这项小型研究就排除阿司匹林可能产生作用的可能性是完全错误的。  
...
15.4 (a) 对两组进行配对 $t$ 检验，结果为 $t = 4.23$（8 自由度；$\mathbf{P} = 0.003$）和 $t = 1.58$（6 自由度；$\mathbf{P} = 0.17$）。仅在一组中出现高度显著的变化可能表明各组之间确实存在差异。然而，这些是组内分析，而临床试验的全部目的是直接比较各组。我们应该使用双样本 $t$ 检验对治疗一周后的值或收缩压的变化进行分析，而不是通过独立分析中 $\mathbf{P}$ 值的间接比较。作者的解释无效。  
...
(b) 两组变化的均值（标准差）分别为 12.67 (8.99) 和 4.71 (7.91)，$t$ 检验结果为 $t = 1.85$（14 自由度；$\mathbf{P} = 0.09$）。（同样，对一周血压的比较结果为 $t = 1.65$（14 自由度；$\mathbf{P} = 0.12$）。）因此，存在一些微弱的证据表明各组可能存在差异—作者得出的结论并未得到正确分析的支持。  
...
(d) 经过Yates校正的卡方检验结果为 $X^{2} = 3.89$（$\mathbf{P} = 0.049$）。引用的 $\mathbf{P}$ 值是单侧的，尽管论文中没有对此进行说明。因此，结果仅具有边缘统计学意义。RR为0.33，其 $95\%$ 置信区间非常宽，从0.12到0.93。作者谨慎解释他们的发现是正确的，并建议需要进行更大规模的试验来确认（或不确认）阿司匹林在这种情况下是否有效。  
...
16.1 $\mathbf{P}$ 值是衡量反对零假设证据强度的指标。它不表示观察到的效应大小。两个不同规模的临床试验可能产生相同的治疗效应，但 $\mathbf{P}$ 值不同。对于连续变量，$\mathbf{P}$ 值也取决于变异性（参见练习9.6）。  
...
16.2 如果我们不认为“相同”指的是样本量，那么一个原因可能是研究患者数量的差异。两个规模相同的“相同”研究预计不会产生完全相同的结果。如果研究规模较大，观察到的效应（和 $\mathbf{P}$ 值）往往会更接近，而不是规模较小。大型研究中出现显著不同的结果可能意味着这些研究并非如声称的那样“相同”。例如，不同国家或不同实验室的患者之间可能存在差异。  
...
16.3 (a) 假设为正态分布，男性的平均身高可以表示为比女性平均身高高出 (179.1 - 171.7)/5.75 个标准差。该值为 1.287。从表 B1 可知，对应于标准正态离差 1.25 和 1.30 的上尾面积分别为 0.1056 和 0.0968，因此所需值为 0.10 或  $10\%$ 。  
...
16.4 人口的年龄结构发生了显著变化，老年人口的比例远高于过去。总死亡率，通过将年龄别死亡率与风险人数相乘计算得出，保持不变，因为年龄别死亡率的下降被老年人口数量的增加所抵消。  
...
16.5 (a) 否，因为这些数字是百分比。尽管卡方检验是一种比较比例（或百分比）的方法，但它必须对频数进行。  
...
(b) 百分比分别为  $73 / 457 = 16.0\%$  和  $138 / 370 = 37.3\%$ ，因此差异为  $21\%$ 。 $95\%$  置信区间为  $15\%$  到  $27\%$ 。由于样本量大，该区间相当窄。  
...
(c) 评估趋势最简单的方法是使用卡方趋势检验。为三个社会阶层组赋予等间距的分数（例如  $-1$ 、0 和 1）是合理的。那些社会阶层未分类的儿童必须被排除。所比较的比例在问题开始时已给出。卡方检验结果为  $X_{\text{trend}}^{2} = 3.64 \quad (\mathbf{P} = 0.06)$  和  $X_{\text{trend}}^{2} = 7.69$ $(\mathbf{P} = 0.006)$ 。  
...
(d) 尽管在一个组中趋势显著（在  $5\%$  水平），而在另一个组中不显著，我们不应推断这种关系仅存在于非氟化区域。比较趋势最简单的方法是使用回归方法估计每个社会阶层类别变化中  $\%$  dmft 的变化（第 11.15.2 节），并比较斜率（第 11.12.1 节）。或者，所有数据可以一次性通过复杂的逻辑回归分析进行分析。  
...
(e) 这种效应异质性的专业术语是“交互作用”。  
(e) The technical term for such a heterogeneity of effect is 'interaction'.  

16.6 (a) 间接测量值与直接测量值之差对其平均值的散点图显示，这些差异与血压水平之间没有关联趋势。差异的均值和标准差分别为 $-4.80$ 和 12.46，因此 95% 一致性限度为 $-29.72$ 至 $20.12 \mathrm{mmHg}$。  
...
(b) 值 $-10$ 和 $+10$ 分别距离均值 $-0.42$ 和 1.18 个标准差。假设差异来自正态总体，则任一方向上差异大于 10 的概率是小于 $-10$ 的概率加上大于 10 的概率，这（根据表 B1）是 $0.3372 + 0.1190 = 0.4562$，或大约 46%。或者，我们可以将观察到的比例作为估计值，即 $22 / 50 = 0.44$ 或 44%。(当差异呈正态分布时，此方法可靠性较低。)  
...
(c) 差异与体重的 Pearson 相关系数为 0.32 $(\mathbf{P}< 0.01)$，与臂围为 0.18 $(\mathbf{P} > 0.10)$，与年龄为 $-0.24 (\mathbf{P} > 0.05)$。因此，有一定证据表明直接测量值与间接测量值之间的差异随体重增加而增大。回归线的斜率为 0.394 $(\mathbf{SE} = 0.167)$，表明每增加 $10 \mathrm{kg}$ 体重，两种方法之间的差异估计增加 $4 \mathrm{mmHg}$。  
...
(d) 这可能是由于新的直接测量方法存在“学习效应”。差异与测量顺序之间的秩相关系数为 0.37 $(\mathbf{P}< 0.01)$。  
...
(e) 最后 40 名女性测量值差异的均值和标准差分别为 $-2.93$ 和 $11.47 \mathrm{mmHg}$，因此 95% 一致性限度为 $-25.87$ 和 20.01。这些限度与使用所有 50 名女性数据得出的限度相比，并没有明显缩小。  
...
<table><tr><td></td><td>均值 (Mean)</td><td>标准差 (SD)</td></tr><tr><td>第 1 天 (Day 1)</td><td>151.27</td><td>62.37</td></tr><tr><td>第 2 天 (Day 2)</td><td>194.40</td><td>78.77</td></tr><tr><td>第 3 天 (Day 3)</td><td>111.47</td><td>61.14</td></tr></table>  
...
由于标准差相似，参数分析的假设很可能得到满足，但在拟合模型后应检查残差。方差分析中三天时间的比较结果为 $F = 7.24$，自由度为 2 和 28 $(\mathbf{P} = 0.003)$。此模型残差的 Shapiro-Francia $W^{\prime}$ 检验结果为 $W^{\prime} = 0.992$ $(\mathbf{P} = 0.97)$，表明残差分布非常接近正态分布。  
...
由于各天之间存在高度显著的变异，因此使用基于方差分析残差标准差（59.7175 mmol/l）的 SE 进行 $t$ 检验来检查每对天是合理的。这些检验结果如下：  
...
<table><tr><td></td><td>差异</td><td>t</td><td>P</td><td>P*</td></tr><tr><td>第1天 对 第2天</td><td>43.13</td><td>1.98</td><td>0.06</td><td>0.17</td></tr><tr><td>第1天 对 第3天</td><td>-39.80</td><td>-1.82</td><td>0.08</td><td>0.24</td></tr><tr><td>第2天 对 第3天</td><td>-82.93</td><td>-3.80</td><td>0.0007</td><td>0.002</td></tr></table>  
...
其中 $\mathbf{P}^{*}$ 是 $\mathbf{P}$ 乘以 3（Bonferroni 校正）。即使经过 Bonferroni 校正，第2天和第3天之间也存在高度显著性差异。其他差异不显著，但血浆醛固酮的平均变化相当大。  
...
(b) 残差标准差也应用于构建任意两对均值之间差异的置信区间。第1天和第2天均值之间差异的 $95\%$ 置信区间为 $43.13 \pm 2.048 \times 59.7175 \times \sqrt{\frac{1}{13} + \frac{1}{13}}$，即 $-1.53$ 至 $87.79 \mathrm{mmol / l}$。这个宽泛的置信区间表明，血浆醛固酮可能确实存在与从低海拔快速变化到高海拔相关的真实变化，但需要更大规模的研究来探究这种可能性。  
...
(c) 高山病评分（AMS）与血浆醛固酮变化之间的相关系数为 $-0.36$ $(\mathbf{P} = 0.19)$。几乎没有证据表明这两个变量之间存在关联。  
...
16.8 不，这不是一个合理的论点。置信区间确实会很宽。这并非没有意义，而是表明研究规模太小，无法得出精确的结论。  
